function f19 = f_hmhr_res_rhor_2tr(tr, rhor, para)
n(40) = para(8);
n(41) = - para(8);
n(42) = para(9);
n(43) = - para(9);
...
f19 = 0;
for i = 1: 7
    f19 = f19 + n(i) * d(i) * tt(i) * (tt(i) - 1) * rhor^(d(i) - 1) * ...
        tr^(tt(i) - 2);
end
for i = 8: 34
    f19 = f19 + n(i) * tt(i) * (tt(i) - 1) * rhor^d(i) * ...
        tr^(tt(i) - 2) * exp(- rhor^c(i)) * (d(i) / rhor - c(i) * ...
        rhor^(c(i) - 1));
end
for i = 35: 45
    f19 = f19 + n(i) * rhor^d(i) * tr^tt(i) * exp(- alpha(i) * ...
        (rhor - epsilon(i))^2 - beta(i) * (tr - gama(i))^2) * ...
        ((tt(i) / tr - 2 * beta(i) * (tr - gama(i)))^2 - ...
        tt(i) / tr^2 - 2 * beta(i)) * (d(i) / rhor - 2 * alpha(i) * ...
        (rhor - epsilon(i)));
end