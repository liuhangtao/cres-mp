function f32 = f_kernel_tr(tr, rhor, const, para)
alpha = const(4);
beta = const(5);
gama = const(6);
deta1 = const(7);
...
f32 = a20 * (1 / tr^2 - 1 / tr^3) * (y^(- alpha / deta1) - 1) - ...
    a20/2 *(1 / tr - 1)^2*alpha/deta1*y^(- alpha / deta1 - 1)*y_tr + ...
    a21 * (1 / tr^2 - 1 / tr^3) * (y^(- (alpha - deta1) / deta1) - 1) -... 
    (alpha - deta1)/2/deta1*a21*(1 / tr - 1)^2 * y^(- alpha / deta1) * y_tr;