function f15 = f_hmhr_res_2rhor(tr, rhor, para)
n(40) = para(8);
n(41) = - para(8);
n(42) = para(9);
n(43) = - para(9);
...
f15 = 0;
for i = 1:7
    f15 = f15 + n(i) * d(i) * (d(i) - 1) * rhor^(d(i) - 2) * tr^tt(i);
end
for i = 8:34
    f15 = f15 + n(i) * rhor^(d(i) - 2) * tr^tt(i) * exp(- rhor^c(i)) * ...
        ((d(i) - c(i) * rhor^c(i)) * (d(i) - 1 - c(i) * rhor^c(i)) - ...
        c(i)^2 * rhor^c(i));
end
for i = 35: 45
    f15 = f15 + n(i) * tr^tt(i) * exp(- alpha(i) * ...
        (rhor - epsilon(i))^2 - beta(i) * (tr - gama(i))^2) * (- 2 * ...
        alpha(i) * rhor^d(i) + 4 * alpha(i)^2 * rhor^d(i) * ...
        (rhor - epsilon(i))^2 - 4 * d(i) * alpha(i) * rhor^(d(i) - 1) * ...
        (rhor - epsilon(i)) + d(i) * (d(i) - 1) * rhor^(d(i) - 2));
end