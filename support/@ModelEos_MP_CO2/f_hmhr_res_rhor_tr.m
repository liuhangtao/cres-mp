function f9 = f_hmhr_res_rhor_tr(tr, rhor, para)
n(40) = para(8);
n(41) = - para(8);
n(42) = para(9);
n(43) = - para(9);
...
f9 = 0;
for i = 1:7
    f9 = f9 + n(i) * d(i) * tt(i) * rhor^(d(i) - 1) * tr^(tt(i) - 1);
end
for i = 8:34
    f9 = f9 + n(i) * tt(i) * rhor^(d(i) - 1) * tr^(tt(i) - 1) * ...
        exp(- rhor^c(i)) * (d(i) - c(i) * rhor^c(i));
end
for i = 35: 45
    f9 = f9 + n(i) * rhor^d(i) * tr^tt(i) * exp(- alpha(i) * ...
        (rhor - epsilon(i))^2 - beta(i) * (tr - gama(i))^2) * ...
        (d(i) / rhor - 2 * alpha(i) * (rhor - epsilon(i))) * ...
        (tt(i) / tr - 2 * beta(i) * (tr - gama(i)));
end